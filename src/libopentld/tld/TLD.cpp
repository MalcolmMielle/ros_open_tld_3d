...
/**********************************µMY VERSION*********************************/
void TLD::coninueSelectObject(const Mat &img, Rect *bb)
{
...
void TLD::continueLearning()
{
    ...
    for(int i = 0; i < detectorCascade->numWindows; i++)
    {
        ...
        if(overlap[i] < 0.2)
        {
            float variance = detectorCascade->detectionResult->variances[i];
    ...
    for(int i = 0; i < numIterations; i++)
    {
        ...
        //Learn this bounding box
        //TODO: Somewhere here image warping might be possible
        detectorCascade->ensembleClassifier->learn(&detectorCascade->windows[TLD_WINDOW_SIZE * idx], true, &detectorCascade->detectionResult->featureVectors[detectorCascade->numTrees * idx]);
    }
...
void TLD::writeToFile(std::string& path)
{
...
void TLD::readFromFile(std::string& path)
{
    release();
    INNClassifier *nn = detectorCascade->nnClassifier;
    IEnsembleClassifier *ec = detectorCascade->ensembleClassifier;
    ...
    if(file == NULL)
    {
        ROS_ERROR("Error: Model not found: %s\n", path.c_str());
        exit(1);
    }
